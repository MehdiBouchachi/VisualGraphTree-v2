...
export default function TP3Sorts() {
  ...
  // Algorithm settings (PDF-style QuickSort is fixed; only order is selectable)
  const [order, setOrder] = useState("asc");

  // Run quicksort (pivot = first, Hoare)
  const result = useMemo(() => {
    return quickSortInstrumented(arr, { order, record: true });
  }, [arr, order]);

  const { steps = [], stats = {}, tookMs = 0 } = result;
  ...
  return (
    <div className="grid gap-6">
      ...
      <Card>
        <Title>
          TP3 – Tri rapide (version PDF : pivot premier, partition Hoare)
        </Title>
        ...
        <Row role="group" aria-label="Sorting controls">
          ...
          <Field htmlFor="n">
            <span className="field-label">Taille du tableau (N)</span>
            <Input
              id="n"
              value={nStr}
          ...
          <Field htmlFor="min">
            <span className="field-label">Valeur min</span>
            <Input
              id="min"
              value={minStr}
          ...
          <Field htmlFor="max">
            <span className="field-label">Valeur max</span>
            <Input
              id="max"
              value={maxStr}
        ...
        >
          <ButtonGroup>
            <Button onClick={regenerate}>Générer</Button>
            <Button
              secondary
              onClick={() =>
                buildFromText(
                  prompt(
                    "Entrez des nombres (séparés par espace ou virgule) :"
                  ) ?? ""
                )
              }
            >
      ...
      <Card>
        ...
        >
          ...
          >
            <span style={{ fontSize: 12, color: "#64748b" }}>Vitesse</span>
            <input
              type="range"
              min="40"
              max="800"
      ...
      <Card>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-3">
          <DataItem label="Comparaisons">{stats.comparisons}</DataItem>
          <DataItem label="Échanges">{stats.swaps}</DataItem>
          <DataItem label="Partitions">{stats.partitions}</DataItem>
          <DataItem label="Profondeur max">{stats.maxDepth}</DataItem>
          <DataItem label="Temps d'exécution">
            {(tookMs ?? 0).toFixed(2)} ms
          </DataItem>
          <DataItem label="Taille">{(steps[0]?.a ?? arr)?.length}</DataItem>
          <DataItem label="Ordre">{order}</DataItem>
        </div>
      </Card>
    </div>
  );
